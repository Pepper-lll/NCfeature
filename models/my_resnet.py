...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, use_norm=False, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, fix_dim=False, ETF_fc=False):
    ...
    def forward_embedding(self, x):
        # feature before classifier layer
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
