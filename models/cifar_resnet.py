...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, use_norm=False, fix_dim=False, ETF_fc=False):
        ...
        if not fix_dim:
            ...
            self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2, outdim=num_classes)
            ...
            if isinstance(m, nn.Conv2d):
                ...
                if ETF_fc:
                    ...
                    if fix_dim:
                        ...
                        m.weight = nn.Parameter(torch.mm(weight, torch.eye(num_classes, 64)))
...
def resnet32(num_classes=10, use_norm=False, ETF_fc=False):
    return ResNet_s(BasicBlock, [5, 5, 5], num_classes=num_classes, use_norm=use_norm, ETF_fc=ETF_fc)
