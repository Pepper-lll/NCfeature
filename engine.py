...
import time
import torch
from losses import NC2Loss, NC2Loss_v1, NC2Loss_v2, zero_center
...
def train(train_loader, model, criterion_sup, criterion_ft1, \
    ...
    Args:
        ...
        model (nn.Module / nn.DataParallel.module): Model to train.
        criterion_sup (): loss function for supervised learning.
        criterion_ft1 (): loss function for feature supervision.
        optimizer_model (torch.optim.Optimizer): Optimizer for model.
        optimizer_ftloss (torch.optim.Optimizer): Optimizer for feature supervision.
        epoch (int): Current epoch.
        log (_type_): Log handler.
        tf_writer (_type_): Tensorboard writer.
        args (_type_): Arguments.

    Returns:
        losses.avg (float): Average loss.
        suploss.avg (float): Average supervised loss.
        ftloss1.avg (float): Average NC1 feature loss.
        ftloss2.avg (float): Average NC2 feature loss.
        max_coses.avg (float): Average cosine similarity.
        all_targets (np.array): All targets.
        all_preds (np.array): All predictions.
        M (np.array): M matrix.
        c_means (np.array): Center means.
    """
    ...
    for i, (input, target, index) in enumerate(train_loader):
        ...
        # forward pass, get feature and logit_outputs
        feature, output = model(input)
        H.append(feature.detach())
        _, preds = torch.max(output, 1)
        all_preds.extend(preds.cpu().numpy())
        # do logits adjustment if needed
        if args.logit_adj_train:
            output += args.logit_adjustments
        
        # [loss] compute class supervision loss
        ...
        # [loss] compute NC1 loss, and fetch the learned means
        ft_loss1, c_means = criterion_ft1(feature, target)
        
        # [loss] compute NC2 loss, and fetch the max_cos(equals to min angle), main branch v0
        ...
        for c in range(C):
            ...
            if len(idxs) == 0: # If no class-c in this batch
                continue
            h_c = feature[idxs,:] # (num_sample_belonging_cls, feat_dim)
            mean[c] += torch.sum(h_c, dim=0) # feat_dim
        ...
        # measure elapsed time; logging per iteration
        ...
        if i % args.print_freq == 0:
            output = ('\n Epoch: [{0}][{1}/{2}], lr: {lr:.6f}\t'
                ...
                loss=losses, sup_loss=sup_losses, ft_loss1=ft_losses1, ft_loss2=ft_losses2,
    ...
    if log:
        epoch_out = ('\n ** Epoch: [{0}], lr: {lr:.6f}\t'
                      ...
                      'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                      'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                epoch, loss=losses, sup_loss=sup_losses, ft_loss1=ft_losses1, ft_loss2=ft_losses2,
                max_cos = max_coses, top1=top1, top5=top5, lr=optimizer_model.param_groups[-1]['lr']))  # TODO
        print(epoch_out)
        log.write(epoch_out + '\n')
        log.flush()
