...
def train(train_loader, model, criterion_sup, criterion_ft1, \
    ...
    for i, (input, target, index) in enumerate(train_loader):
        ...
        elif args.NC2Loss == 'v2': # cosine to -1/(k-1)
        ...
        for c in range(C):
            ...
            Var[c] += (torch.norm(h_c - c_means[c], dim=1) **2).sum()
            N[c] += h_c.shape[0]
        ...
        if (args.lamda1 > 0. or args.lamda2 > 0.) and optimizer_ftloss:
        ...
        if i % args.print_freq == 0:
            output = ('\n Epoch: [{0}][{1}/{2}], lr: {lr:.6f}\t'
                      ...
                      'Zero_Center_reg {center.val:.4f} ({center.avg:.4f})\t'
    ...
    for c in range(C):
        ...
        Var[c] = round(Var[c].item(), 4)
    M = torch.stack(mean)
    Mnorms = torch.norm(M, dim=1)
    g_norm = M.mean(dim=0).norm()
    M_ = M - torch.mean(M, dim=0, keepdim=True)
    M_np = M_.detach().cpu().numpy()
    # eigvec, eigval, _ = svds(M_np, k=C-1)
    print(' *NC1* avg distance of features to class-mean:', dis)
    print(' *NC1* Variance of features to class-mean:', Var)
    # print(' *NC2* eigen value of M_hat', eigval)
    M_norms = torch.norm(M_, dim=1)
    M_coV = round((torch.std(M_norms)/torch.mean(M_norms)).item(), 4)
    print(' *NC2* CoV of centered feature means:', M_coV)
    print('norm of global mean:', g_norm)
    print(' *NC2* norm of class means:', Mnorms.detach().cpu().numpy())
    print(' *NC2* norm of centered means:', M_norms.detach().cpu().numpy())
    M_dist = (torch.norm(c_means - M, dim=1)**2).detach().cpu().numpy()
    print('distance between updated batch means and total mean:', M_dist)
    ...
    if tf_writer:    
        ...
        # tf_writer.add_scalar('analyze/glbal_mean_norm', g_norm, epoch)
    
    H = torch.cat(H).detach()
    if args.arch =='resnet32':
        if args.gpu is None:
            W = model.module.linear.weight.detach()
            b = model.module.linear.bias.detach()
        else:
            W = model.linear.weight.detach()
            b = model.linear.bias.detach()
    else:
        W = model.module.fc.weight.detach()
        b = model.module.fc.bias.detach()
    Wnorms = torch.norm(W, dim=1)
    print('norm of classifiers:', Wnorms.detach().cpu().numpy())
    W_coV = round((torch.std(Wnorms)/torch.mean(Wnorms)).item(), 4)
    print('CoV of classifier norms:', W_coV)
...
def validate(val_loader, model, criterion_sup, epoch, tf_writer, args, cls=False, flag='val'):
    ...
    for c in range(C):
        ...
        # dis[c] /= N[c]
        # dis[c] = round(dis[c].item(), 4)
    # print('avg distance of features to class-center:', dis) 
    M = torch.stack(mean)
    H = torch.cat(H).detach()
    Y = torch.from_numpy(np.array(all_targets)).cuda()
    ...
    if tf_writer:
        ...
        tf_writer.add_scalars('acc/test_' + flag + '_cls_acc', {str(i):x for i, x in enumerate(cls_acc)}, epoch+args.epochs if cls else epoch)
